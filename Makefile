...
output: env
	@echo -e $(bold)$(primary)early_warning_service_arn$(clear) = $(shell terraform -chdir=plans output early_warning_service_arn)
	@echo -e $(bold)$(primary)early_warning_service_role$(clear) = $(shell terraform -chdir=plans output early_warning_service_role)
	@echo -e $(bold)$(primary)early_warning_service_role_arn$(clear) = $(shell terraform -chdir=plans output early_warning_service_role_arn)
	@echo -e $(bold)$(primary)early_warning_service_policy_arn$(clear) = $(shell terraform -chdir=plans output early_warning_service_policy_arn)
...
tail-aws-logs: ## Install using pipx install awscliv2
	awsv2 logs tail "/aws/lambda/$(shell sed -e 's/\(.*\)/\L\1/' <<< "${APP_ENV}")-early-warning-service" --follow
