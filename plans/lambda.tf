resource "aws_lambda_function" "early_warning_service" {
  ...
  role          = aws_iam_role.early_warning_service_role.arn
  handler       = "app.handler"
...
resource "aws_lambda_permission" "allow_queue" {
  ...
  source_arn    = aws_sqs_queue.early_warning_service_queue.arn
}
